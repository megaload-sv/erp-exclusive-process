...
<div class="studio-shell">
    <aside class="studio-sidebar">
        <div class="studio-brand">
            ...
            <div>
                <strong>TraceOps Studio</strong>
                <small><?= esc($runtimeVersion ?? 'Runtime') ?></small>
            </div>
        </div>
        ...
        <nav class="studio-navigation" aria-label="Navegación de TraceOps Studio">
            ...
            <?php foreach ($items as $item): ?>
                ...
                <?php if ($item['enabled']): ?>
                    <a class="studio-nav-item <?= $active ? 'is-active' : '' ?>" href="<?= site_url('developer') ?>">
                        <span class="studio-nav-icon"><?= esc($item['icon']) ?></span>
                        <span><?= esc($item['label']) ?></span>
                    </a>
                <?php else: ?>
                    <span class="studio-nav-item is-disabled" aria-disabled="true">
                        <span class="studio-nav-icon"><?= esc($item['icon']) ?></span>
                        <span><?= esc($item['label']) ?></span>
        ...
        <div class="studio-sidebar__footer">
            <span>Studio v0.2 Alpha</span>
            <a href="<?= site_url('/') ?>">Volver al ERP</a>
        </div>
    </aside>
    ...
    <main id="studio-content" class="studio-main" tabindex="-1">
        <header class="studio-topbar">
            <div>
                ...
                <h1><?= esc($title ?? 'TraceOps Studio') ?></h1>
            </div>
